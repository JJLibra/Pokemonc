...
int main(int argc, char **argv) {
    ...
    struct arguments arguments = {NULL, "regular", 0, 0, 0, 0, 0, 0, config->version};
    if (argc == 1) {
        print_usage(config);
        free_config(config);
        return 0;
    ...
    if (arguments.list) {
        ...
        display_random_pokemon(pokemon_list, pokemon_count, arguments.shiny, arguments.no_title, arguments.no_mega, arguments.no_gmax);
